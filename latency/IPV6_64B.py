...
SRC_IP6 = "2001:db8:10::1"      # IPv6 source (TX side)
DST_IP6 = "2001:db8:20::1"      # IPv6 destination (RX side)
...
PPS_LIST   = [100, 1000, 10000]                  # Packet rates per second

CSV_FILE = "trex_iter_results_ipv6_udp.csv"      # Output CSV file
MACS_CSV = "macs.csv"                            # Input CSV generated by mac_to_csv.sh

# ---- Frame sizing (Ether + IPv6 + UDP + Payload) ----
ETHER_HDR = 14
IPV6_HDR  = 40
UDP_HDR   = 8
LAT_PAYLOAD_SIZE = 18                            # Space for latency stamping
...
def read_macs_from_csv(path):
    """
    Read TX/RX MAC addresses from the macs.csv file created by mac_to_csv.sh.
    ...
    Logic:
      - If PCI ends with '.0' and '.1' → use .0 as TX and .1 as RX
      - Otherwise → use first two rows
    """
    ...
    # Fallback: take first two rows if no .0/.1 pattern found
...
def build_pkt():
    ...
    if l2_len != expected_l2:
        raise RuntimeError(f"L2 length is {l2_len} bytes, expected {expected_l2}.")
...
# ------------------------------------------------------
# Run one iteration of experiment
# ------------------------------------------------------
...
def main():
    ...
    # Step 1: Read MACs from macs.csv (if present)
    ...
    # Step 2: Ensure CSV header and connect to TRex
