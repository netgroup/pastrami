...
# --------------------
# Default MACs (will be overwritten if macs.csv exists)
# --------------------
...
CSV_FILE = "trex_iter_results_1500B_ipv4_fix.csv"   # Results output CSV
MACS_CSV = "macs.csv"                                # Input file with MACs

# ---- Packet sizing: exactly 1500B on the wire (1496B L2 + 4B FCS) ----
TARGET_WIRE_BYTES = 1500
FCS_BYTES         = 4
TARGET_L2_BYTES   = TARGET_WIRE_BYTES - FCS_BYTES  # 1496 bytes at L2 level

ETHER_HDR = 14
IP_HDR    = 20
UDP_HDR   = 8

PAYLOAD_SIZE = TARGET_L2_BYTES - (ETHER_HDR + IP_HDR + UDP_HDR)  # 1454 bytes payload
if PAYLOAD_SIZE < 0:
    raise RuntimeError("Negative payload size! Check header sizes.")
PAYLOAD = b'X' * PAYLOAD_SIZE  # 1454B raw payload


def read_macs_from_csv(path):
    """
    Reads MAC addresses from macs.csv.
    CSV format example:
    ...
    Selection rules:
      - If PCI addresses end with '.0' and '.1', use them as TX and RX respectively
      - Otherwise, use the first two rows as TX and RX
    """
...
def build_pkt():
    """Build one IPv4/UDP Ethernet frame with exact L2 size TARGET_L2_BYTES (no FCS)."""
    scapy_pkt = (
    ...
    if len(scapy_pkt) != TARGET_L2_BYTES:
        raise RuntimeError("L2 length is {} bytes, expected {} for {}B-on-wire."
                           .format(len(scapy_pkt), TARGET_L2_BYTES, TARGET_WIRE_BYTES))
    return STLPktBuilder(pkt=scapy_pkt)


def ensure_csv_header(path):
    """Create results CSV file if missing and add the header row."""
...
def one_iteration(c, pps, burst, iteration_idx):
    """Run one iteration of the latency test for a specific (pps, burst)."""
    ...
    if short_run:
        c.start(ports=[TX_PORT], duration=1.0)
    else:
...
def main():
    ...
    # Step 1: Read MAC addresses from macs.csv in the same directory
