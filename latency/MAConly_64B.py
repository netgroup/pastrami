...
from trex_stl_lib.api import *     # TRex STL API
from scapy.all import Ether, Raw   # L2-only packet construction

# --------------------
# Default test parameters (MACs will be replaced if macs.csv exists)
# --------------------
SRC_MAC = "00:00:00:00:11:11"  # Default L2 source MAC
DST_MAC = "00:00:00:00:22:22"  # Default L2 destination MAC
ETH_TYPE = 0x88B5               # Custom EtherType (non-IP)

TX_PORT = 0                     # TRex TX port index
RX_PORT = 1                     # TRex RX port index
PGID    = 5                     # Latency flow-stat group ID

ITERATIONS = 7                                   # Number of runs per (burst, pps)
BURSTS     = [1000, 10000, 100000]               # Packets per burst
PPS_LIST   = [100, 1000, 10000]                  # Packets per second

CSV_FILE = "trex_iter_results_64B_L2only.csv"    # Output CSV
MACS_CSV = "macs.csv"                            # Input CSV (from mac_to_csv.sh)

# ---- Frame sizing: exactly 64 bytes on the wire (60B L2 + 4B FCS) ----
TARGET_WIRE_BYTES = 64
FCS_BYTES         = 4
TARGET_L2_BYTES   = TARGET_WIRE_BYTES - FCS_BYTES  # 60 bytes at L2 (without FCS)

ETHER_HDR = 14                                   # Ethernet header length
PAYLOAD_SIZE = TARGET_L2_BYTES - ETHER_HDR       # 60 - 14 = 46B payload
if PAYLOAD_SIZE < 0:
    raise RuntimeError("Negative payload size! Check header lengths.")
PAYLOAD = b'X' * PAYLOAD_SIZE                    # 46 bytes of raw data


def read_macs_from_csv(path):
    """
    Read TX/RX MAC addresses from a CSV file generated by mac_to_csv.sh.
    CSV format:
    ...
    Selection logic:
      - If PCI entries end with '.0' and '.1' → use .0 for TX and .1 for RX
      - Otherwise → use the first two rows
    """
    ...
    # Fallback to first two rows if PCI names are missing
...
def build_pkt():
    """Build a pure Ethernet frame (L2 only, no IP/UDP) with fixed 60B length."""
    scapy_pkt = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=ETH_TYPE) /
        Raw(PAYLOAD)
    )
...
def ensure_csv_header(path):
    """Create the output CSV with header row if it doesn't exist yet."""
...
def one_iteration(c, pps, burst, iteration_idx):
    """Run a single iteration of the test for given (pps, burst)."""
    ...
    row = {
        ...
        "avg_latency_us": avg_us, "min_latency_us": min_us,
        "max_latency_us": max_us, "jitter_us": jitter_us,
        "drops": errc.get('dropped', 0), "ooo": errc.get('out_of_order', 0),
        "dup": errc.get('dup', 0), "seq_too_high": errc.get('seq_too_high', 0),
...
def main():
    ...
    # Step 1: Read MACs from macs.csv in the current directory
    ...
    # Step 2: Ensure output CSV and start TRex client
