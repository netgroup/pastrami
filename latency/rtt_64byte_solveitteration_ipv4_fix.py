...
CSV_FILE = "trex_iter_results_64B_ipv4_fix.csv"      # Results CSV path
...
# Payload size so that L2 length (without FCS) is exactly 60 bytes:
PAYLOAD_SIZE = 60 - (ETHER_HDR + IP_HDR + UDP_HDR)   # 60 - (14+20+8) = 18 bytes
if PAYLOAD_SIZE < 0:
    raise RuntimeError("Negative payload size! Check header sizes.")  # Sanity guard
PAYLOAD = b'X' * PAYLOAD_SIZE  # Raw payload bytes

def build_pkt():
    """Build a single IPv4/UDP Ethernet frame with L2 length 60B (FCS excluded)."""
    scapy_pkt = (
        ...
        Raw(PAYLOAD)                                    # Raw payload (18B)
    )
    # Scapy len() returns L2 length excluding FCS; expect 60 for 64B on wire.
    l2_len = len(scapy_pkt)                              # Compute L2 frame size
    if l2_len != 60:
        raise RuntimeError("L2 length is {} bytes, expected 60 for 64B-on-wire.".format(l2_len))
