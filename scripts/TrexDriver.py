...
#sys.path.insert(0, "/opt/trex-core-3.04/scripts/automation/trex_control_plane/interactive/")
sys.path.insert(0, "/opt/v3.06/automation/trex_control_plane/interactive")
from trex_stl_lib.api import *

SCRIPT_DIR = Path(__file__).resolve().parent
PCAP_ABS = (SCRIPT_DIR / "pcap/trex-pcap-files/plain-ipv6-64.pcap").as_posix()
class TrexOutput():
    ...
    def setTxPortInfo(self, tInfo):
        self.output['tx']['port_info'] = tInfo

    def setRxPortInfo(self, tInfo):
        self.output['rx']['port_info'] = tInfo

    def setTxStats(self, tStats):
        self.output['tx']['stats'] = tStats

    def setRxStats(self, tStats):
        self.output['rx']['stats'] = tStats
    ...
    def setPgIdStats(self, pgid_stats):
        self.output['pgid_stats'] = pgid_stats
    ...
    def getTxPortInfo(self):
        return self.output['tx']['port_info']

    def getRxPort(self):
        return self.output['rx']['port_info']

    def getTxStats(self):
        return self.output['tx']['stats']

    def getRxStats(self):
        return self.output['rx']['stats']
    ...
    def getPgIdStats(self):
        return self.output['pgid_stats']
...
###            ipv6_filter = STLFilter(ether_type=0x86DD)
...
###            client.set_rx_filter(port=1, filter=ipv6_filter)
            ...
            # We retrieve informations from Tx and Rx ports.
            txInfo = client.get_port_info(self.txPort)
            rxInfo = client.get_port_info(self.rxPort)

            tOutput.setTxPortInfo(txInfo)
            tOutput.setRxPortInfo(rxInfo)

            # We retrieve statistics from Tx and Rx ports.
            txStats = client.get_xstats(self.txPort)
            rxStats = client.get_xstats(self.rxPort)

            tOutput.setTxStats(txStats)
            tOutput.setRxStats(rxStats)

            tOutput.setPgIdStats(client.get_pgid_stats())

            # We retrieve statistics per stream from Tx and Rx ports.
#            perStreamStats = client.get_pgid_stats()['flow_stats']

            if (txInfo[0]['driver'] == "net_ixgbe"):
                tOutput.setTxTotalPackets(txStats['tx_total_packets'])
            else:
                tOutput.setTxTotalPackets(txStats['tx_good_packets'])

            if (rxInfo[0]['driver'] == "net_ixgbe"):
                tOutput.setRxTotalPackets(rxStats['rx_total_packets'])
            else:
                tOutput.setRxTotalPackets(rxStats['rx_good_packets'])

            print("tx_packets: %s" % tOutput.getTxTotalPackets())
            print("rx_packets: %s" % tOutput.getRxTotalPackets())
            print("tx_rate: %s" % ((tOutput.getRxTotalPackets()/tOutput.getTxTotalPackets())*100))
...
if __name__ == '__main__':
    ...
    driver = TrexDriver('127.0.0.1', 0, 1, PCAP_ABS, '100%', 10)
    output = driver.run()
